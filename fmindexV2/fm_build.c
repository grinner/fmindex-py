...
#if TESTINFO
typedef struct { // Only for testing: save space
    ulong bucket_compr; 
    ulong bucket_occ;
...
int fm_build(fm_index *index, uchar *text, ulong length) {
    ...
    count_occ(index);  /* counts occurrences */
    ...
    for(i=0; i < index->num_bucs_lev1; i++) { /* compress each bucket */
...
/* 
    Search for a character not present and puts it all s->skip positions in the original text
*/
static int select_subchar(fm_index *s) {
...
void count_occ(fm_index *s) {
    ...
    /* calculating the number instances characters, alpha_size and bool_char_map */
    for (i = 0; i < s->text_size; i++) {
        curchar = s->text[i];
    ...
    /* remap the alphabet */
    suint free = 0; /* first free place in s->char_map */
    for (i =0; i < ALPHASIZE; i++) {
...
/*
    ...
    The remapping could be done simultaneously with 
    the creation of the bwt ( with s->char_map[s->text[s->lf[i]-1]] )
    but more slowly than + 1 second to 9.
    Input:
...
/* 
    It reminds of the positions. Working for multilocate
*/
int compute_locations(fm_index *s) { 
...
int compute_info_superbuckets(fm_index *s) {
  ...
  ulong dim = s->bucket_size_lev1; // not to divide
...
void write_prologue(fm_index *s) {
  ...
  fm_bit_write24(8, s->char_map[s->specialchar]); /* special character */
  fm_bit_write(32, s->skip);
  fm_uint_write(0); /* free space for occ-explicit list 20th byte --*/
  fm_uint_write(0); /* free space to put sb info beginning 25th byte */
  fm_uint_write(0); /* 29 byte start occ*/
  fm_bit_write24(8, s->char_map[s->subchar]); /* character replaced */
...
void write_susp_infos(fm_index *s) {
    ...
    fm_init_bit_writer(write, &unuseful); // I'm going to write where I left space
    fm_uint_write(s->start_positions);
    fm_uint_write(s->start_prologue_info_sb); // init sbuckets info
    ...
    fm_init_bit_writer(write, &s->compress_size); // return to write to the memory in the queue
}


/*
    If s->skip > 1 writes in compressed all the positions contained in 
    s->loc_occ using s->log2textsize bits for each position.
...
void write_locations(fm_index *s) {
    ...
    ulong i;
    fm_init_bit_buffer();
    if(s->skip == 1) { // mark complete file
        for(i=0; i < s->text_size; i++) {
...
/* 
   Free the allocated memory. Is used in case of error 
or the end of the compression.
*/
static void dealloc(fm_index *s) {
    free(s->lf);
