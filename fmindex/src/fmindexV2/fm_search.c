...
static int allocated, used; /* var count used by multiple */
...
/*
 * Count recursive for multilocate. in the pattern if there is an occurence 
 *  of subchar then I have to divide your search with two separate branches.
 */
int
count_row_mu(fm_index * index, uchar * pattern, ulong len, ulong sp,
     ...
     */
    if (ep < sp) {
        return FM_OK;   /* no occurrence */
    }
    ADD_LIST(sp, ep - sp + 1); /* Adds group to the list found */
...
/*
 * Count to search with marking for multi locate the problem with 
 * this type of marking and 'that you have to consider two branches 
 * search every time you meet the character in the replaced 
 * pattern. This division will degrade performance of the count. the 
 * research on the different paths and 'implemented with a recursive call 
 * not very efficient.
 */
int
...
/*
 * Returns the position in the text is stored on compressed in the i-th 
 * position.
 */
static inline void
get_pos(ulong first_row, ulong element, suint step, ulong * pos,
    ...
    for (i = 0; i < element; i++, pos++) {          // Looking around the group
...
multi_locate(ulong sp, ulong element, ulong * positions, fm_index * index) {
    ...
    ulong *elem_array;  // contains the number of elements of the subgroup
    ulong occsp[index->alpha_size];
    ulong occep[index->alpha_size];
    suint *step_array, step;    // steps as above but
    uchar chars_in[index->alpha_size];
    ...
    /* 
       If the last character and s->specialchar 
        ie the line sp and sp + element is compressed between
       index->bwt_occ[index->specialchar]  and 
    ...
    while (recurs < element) {
        ...
        for (j = 0; j < num_char; j++) {
            ...
            elem_array[--recurs] = diff;    /* simulates recursion */
