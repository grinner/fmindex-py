...
typedef struct {
  ...
  ulong occcharinf; 			/* Number occs of the chars  < index->specialchar */
  ulong occcharsup; 			/* Number occs of the chars  <= index->specialchar */
  
  /* Needed by fm_build */
  ulong *loc_occ;				/* Positions of marcked rows */
  ulong *loc_row;				/* rows marked to extract*/
  ulong pos_marked_row_extr;
  ulong log2_row;
} fm_index;
