...
int fm_bwt_uncompress(fm_index * index) {
    ...
    for(i=0; i < index->text_size; i++) {
        index->bwt_occ[index->bwt[i]]++;
    }
    for(i=1; i < ALPHASIZE; i++) {
        index->bwt_occ[i] += index->bwt_occ[i-1];
    }
